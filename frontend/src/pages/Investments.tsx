...
export const INVESTMENT_CATEGORIES: {
...
}[] = [
  {
    ...
    items: [
      ...
      { id: "lci", label: "LCI", defaultRate: 11.5, desc: "Isento IR" },
      { id: "lca", label: "LCA", defaultRate: 11.0, desc: "Isento IR" },
      {
        id: "tesouro_selic",
        label: "Tesouro Selic",
        defaultRate: 13.5,
      ...
      },
      {
        ...
        label: "Debêntures",
        defaultRate: 14.0,
        desc: "Crédito privado",
      },
      { id: "cri", label: "CRI", defaultRate: 12.5, desc: "Isento IR" },
      { id: "cra", label: "CRA", defaultRate: 12.0, desc: "Isento IR" },
      { id: "coe", label: "COE", defaultRate: 10.0, desc: "Estruturado" },
    ],
  },
  {
  ...
  },
  {
    ...
    color: "#f97316",
    items: [
      { id: "fii", label: "FII", defaultRate: 12.0, desc: "Imobiliário" },
      {
        id: "fundo_acoes",
      ...
      { id: "pgbl", label: "PGBL", defaultRate: 10.0, desc: "Previdência" },
      { id: "vgbl", label: "VGBL", defaultRate: 10.0, desc: "Previdência" },
    ],
  },
  {
    group: "Cripto",
...
const CALC_PERIODS = [
  {
    ...
    label: "1 Ano",
    bg: "bg-white",
    textCls: "text-[var(--primary)]",
    badge: "bg-[var(--secondary)] text-[var(--primary)]",
    divider: "border-[var(--black)]",
  },
  {
    key: "5a",
    months: 60,
    ...
    bg: "bg-[var(--secondary)]",
    textCls: "text-[var(--primary)]",
    badge: "bg-[var(--primary)] text-[var(--secondary)]",
    divider: "border-[var(--black)]",
  },
  {
    key: "10a",
...
function compound(
  ...
  if (annualRate <= 0 || months === 0)
    return months === 0 ? P : P + PMT * months;
...
function InvestmentModal({
  ...
  const proj1y = form.amount
    ? compound(Number(form.amount), pmt, Number(form.annualRate || 0), 12)
    : 0;
  const proj5y = form.amount
    ? compound(Number(form.amount), pmt, Number(form.annualRate || 0), 60)
    : 0;
  const proj10y = form.amount
    ? compound(Number(form.amount), pmt, Number(form.annualRate || 0), 120)
    : 0;

  const inputCls =
    "w-full px-4 py-2.5 rounded-lg border-2 border-[var(--black)] bg-white focus:ring-4 focus:ring-[var(--secondary)] outline-none font-bold text-sm text-[var(--primary)] shadow-[var(--neo-shadow-hover)]";
  const labelCls =
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[var(--main-bg)] w-full max-w-xl rounded-2xl border-3 border-[var(--black)] shadow-[8px_8px_0px_0px_#000] overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="bg-[var(--primary)] border-b-3 border-[var(--black)] px-6 py-4 flex items-center justify-between shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-md border-2 border-white/30 text-white hover:bg-white/10 cursor-pointer"
          >
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
            ...
            <div className="space-y-1.5">
              {INVESTMENT_CATEGORIES.map((group) => {
                ...
                const hasSelected = group.items.some(
                  (i) => i.id === form.category,
                );
                return (
                  <div
                    key={group.group}
                    className="border-2 border-[var(--black)] rounded-xl overflow-hidden"
                  >
                    <button
                      type="button"
                      ...
                      style={{
                        background: hasSelected ? group.color : "#fff",
                        color: hasSelected ? "#fff" : "var(--primary)",
                      }}
                    >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-black text-[var(--black-muted)]">
                  R$
                </span>
                <input
                  type="number"
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-black text-[var(--black-muted)]">
                  %
                </span>
              </div>
            </div>
          ...
          {/* Aporte mensal */}
          <div>
            <label className={labelCls}>Aporte mensal (opcional)</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-black text-[var(--black-muted)]">
                R$
              </span>
            ...
            Number(form.annualRate) > 0 && (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { label: "1 Ano", val: proj1y, months: 12 },
                    { label: "5 Anos", val: proj5y, months: 60 },
                    { label: "10 Anos", val: proj10y, months: 120 },
                  ].map(({ label, val, months }) => {
                    const totalInvested = Number(form.amount) + pmt * months;
                    const gain = val - totalInvested;
                    return (
                      <div
                        key={label}
                        className="flex flex-col p-3 bg-white border-2 border-[var(--black)] rounded-xl shadow-[var(--neo-shadow-hover)]"
                      >
                        <span className="text-[9px] font-black text-[var(--black-muted)] uppercase tracking-wider mb-1">
                          {label}
                        </span>
        ...
        <div className="shrink-0 px-6 py-4 border-t-2 border-[var(--black)] bg-[var(--main-bg)] flex gap-3">
          ...
          <button
            type="button"
            disabled={
              saving || !form.name || !form.amount || form.annualRate === ""
            }
            onClick={() => onSave(form)}
...
function InvestmentCard({
  ...
  const pmt     = inv.monthlyAport ? parseFloat(inv.monthlyAport) : 0;
  const proj1y  = compound(amount, pmt, rate, 12);
  const proj5y  = compound(amount, pmt, rate, 60);
  const proj10y = compound(amount, pmt, rate, 120);
  const base1y  = amount + pmt * 12;
  const gain1y  = proj1y - base1y;
  const pct1y   = (gain1y / base1y) * 100;
  ...
  return (
    <div className="group relative flex flex-col bg-white border-2 border-[var(--black)] rounded-[var(--radius-card)] shadow-[var(--neo-shadow)] hover:shadow-[var(--neo-shadow-hover)] hover:translate-y-[2px] hover:translate-x-[2px] transition-all overflow-hidden">
      ...
      <div
        ...
        style={{ background: meta.groupColor }}
      >
        <div className="flex items-center gap-2">
        ...
        {!isGuest && (
          <div className="flex gap-1.5">
            <button
              onClick={onEdit}
              className="p-1.5 rounded-md border border-white/30 bg-white/10 text-white hover:bg-white/25 transition-colors cursor-pointer"
              title="Editar"
            >
            ...
            <button
              onClick={onDelete}
              className="p-1.5 rounded-md border border-white/30 bg-white/10 text-white hover:bg-red-500/80 transition-colors cursor-pointer"
              title="Excluir"
            >
      ...
      <div className="p-4 flex flex-col gap-3">
        {/* Name + notes */}
        <div>
          <p className="text-sm font-black text-[var(--primary)] tracking-tight leading-snug">
            {inv.name}
          </p>
          {inv.notes && (
            <p className="text-xs text-[var(--black-muted)] mt-0.5 truncate">
              {inv.notes}
            </p>
          )}
        ...
        {/* Amount + Rate + optional monthly */}
        <div className={`grid gap-2 ${pmt > 0 ? "grid-cols-3" : "grid-cols-2"}`}>
          <div className="flex flex-col p-3 bg-[var(--main-bg)] border-2 border-[var(--black)] rounded-xl">
            ...
            <span className="text-sm font-black text-[var(--primary)] tracking-tighter">
              {fmt(amount)}
            </span>
          </div>
          <div className="flex flex-col p-3 bg-[var(--main-bg)] border-2 border-[var(--black)] rounded-xl">
        ...
        {/* 1y gain badge */}
        <div
          className={`flex items-center justify-between px-3 py-2.5 rounded-xl border-2 border-[var(--black)] ${pct1y >= 0 ? "bg-emerald-50" : "bg-red-50"}`}
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span
              className={`text-sm font-black ${pct1y >= 0 ? "text-emerald-600" : "text-red-500"}`}
            >
              {pct1y >= 0 ? "+" : ""}
              {fmt(gain1y)}
            </span>
        ...
        {/* Projections */}
        <div>
          <p className="text-[10px] font-black text-[var(--black-muted)] uppercase tracking-widest mb-2">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[
              ...
              const totalAportado = amount + pmt * months;
              const juros = val - totalAportado;
              return (
              <div
                key={label}
                className="flex flex-col items-center p-2.5 border-2 border-[var(--black)] rounded-xl bg-white shadow-[var(--neo-shadow-hover)]"
              >
                <span className="text-[9px] font-black text-[var(--black-muted)] uppercase tracking-wide">
                  {label}
                </span>
                ...
                <span className="text-[9px] font-bold text-[var(--black-muted)] mt-0.5">
                  +{((juros / totalAportado) * 100).toFixed(0)}%
                </span>
              </div>
              );
            })}
...
export function Investments() {
  ...
  const totalProjected1y = useMemo(
    () =>
      investments.reduce(
        (s, i) =>
          s + compound(parseFloat(i.amount), i.monthlyAport ? parseFloat(i.monthlyAport) : 0, parseFloat(i.annualRate), 12),
        0,
      ),
    [investments],
  );
  ...
  const weightedRate = useMemo(() => {
    ...
    return (
      investments.reduce(
        (s, i) => s + parseFloat(i.annualRate) * parseFloat(i.amount),
        0,
      ) / totalInvested
    );
  ...
  const bestInv = useMemo(
    () =>
      investments.reduce<ApiInvestment | null>(
        (best, i) =>
          !best || parseFloat(i.annualRate) > parseFloat(best.annualRate)
            ? i
            : best,
        null,
      ),
  ...
  const annualRate =
    calcCategory === "custom"
      ? customRate
      : (calcMeta.defaultRate ?? customRate);

  const calcResults = useMemo(
    () =>
      CALC_PERIODS.map(
        ({ key, months, label, bg, textCls, badge, divider }) => {
  ...
  const chartData = useMemo(
    () => genChart(principal, monthly, annualRate),
    [principal, monthly, annualRate],
  );
  ...
  const handleAddFromCalculator = () => {
    setEditingInv({
      name: calcMeta.id !== "custom" ? calcMeta.label : "",
      category: calcCategory,
      amount: principal,
      annualRate: annualRate,
      monthlyContribution: monthly > 0 ? monthly : "",
      startDate: new Date().toISOString().slice(0, 10),
      notes: "",
    });
    setModalOpen(true);
  ...
  const handleEdit = (inv: ApiInvestment) => {
    setEditingInv({
      ...
      monthlyContribution: inv.monthlyAport ? parseFloat(inv.monthlyAport) : "",
    ...
    <main className="min-h-screen mx-auto space-y-8">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-2xl border-3 border-[var(--black)] shadow-[8px_8px_0px_0px_#000] p-6 space-y-4">
            ...
            <p className="text-sm font-bold text-[var(--black-muted)]">
              Remover{" "}
              <span className="text-[var(--primary)]">{deleteTarget.name}</span>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-fr">
        ...
        <div className="lg:col-span-2 bg-[var(--secondary)] border-2 border-[var(--black)] rounded-[var(--radius-card)] shadow-[var(--neo-shadow)] overflow-hidden hover:shadow-[var(--neo-shadow-hover)] hover:translate-y-[2px] hover:translate-x-[2px] transition-all">
          <div className="flex flex-col h-full px-6 py-6 relative">
            <span
              aria-hidden
              className="pointer-events-none select-none absolute -right-3 -bottom-5 text-[130px] font-black text-[var(--primary)] opacity-[0.06] leading-none tracking-tighter"
            >
              R$
            </span>
            ...
            <div className="flex-grow flex flex-col justify-center">
              <p className="text-[10px] font-black text-[var(--primary)] opacity-70 uppercase tracking-widest mb-1">
            ...
            <div className="grid grid-cols-2 gap-3 mt-6">
              <div className="flex flex-col p-3 bg-white border-2 border-[var(--black)] rounded-[var(--radius-main)] shadow-[var(--neo-shadow-hover)]">
                <span className="text-[9px] font-black text-[var(--black-muted)] uppercase tracking-wider mb-0.5">
                  Rentab. média
                </span>
          ...
          {bestInv ? (
            <>
              <div className="flex-grow flex flex-col justify-center mb-4">
                ...
                <span className="text-base font-black text-[var(--secondary)] tracking-tight leading-snug">
                  {bestInv.name}
                </span>
            ...
            <div className="flex-grow flex items-center justify-center">
              <p className="text-xs font-bold text-white/50 uppercase text-center">
      ...
      <section className="bg-white border-2 border-[var(--black)] rounded-[var(--radius-card)] shadow-[var(--neo-shadow)] overflow-hidden">
        <div className="bg-[var(--primary)] border-b-2 border-[var(--black)] px-6 py-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <p className="text-[10px] font-extrabold tracking-widest text-[var(--secondary)] uppercase mb-1">
              Ferramenta
            </p>
          ...
          <div className="flex flex-col items-end px-5 py-3 bg-[var(--secondary)] border-2 border-[var(--black)] rounded-[var(--radius-main)] shadow-[var(--neo-shadow-hover)] shrink-0">
            <span className="text-[9px] font-black text-[var(--primary)] opacity-70 uppercase tracking-wider">
              Taxa anual
            </span>
            <span className="text-4xl font-black text-[var(--primary)] tracking-tighter leading-none">
              {annualRate.toFixed(2)}%
            </span>
            <span className="text-[9px] font-black text-[var(--primary)] uppercase tracking-wider mt-0.5">
              {calcMeta.label}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10">
          ...
          <div className="lg:col-span-4 p-6 lg:border-r-2 border-b-2 lg:border-b-0 border-[var(--black)] flex flex-col gap-6 bg-[var(--main-bg)]">
            ...
            <div>
              <label className="text-[10px] font-black uppercase text-[var(--black-muted)] tracking-widest mb-2 block">
              ...
              <div className="grid grid-cols-2 gap-2 max-h-[240px] overflow-y-auto calc-category-scroll rounded-xl border-2 border-[var(--black)] p-3 bg-white">
                {ALL_CATEGORIES.filter((c) => c.id !== "custom")
                  .concat([
                    {
                      id: "custom",
                      label: "Personalizado",
                  ...
                  .map((cat) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background: isActive ? cat.groupColor : "#fff",
                        }}
                      >
                        <span
                          className="text-xs font-black uppercase tracking-wide leading-tight"
            ...
            {(calcCategory === "custom" || !calcMeta.defaultRate) && (
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-black text-white bg-[#FF3B3B] px-2.5 py-1 border-2 border-[var(--black)] rounded shadow-[var(--neo-shadow-hover)]">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-black text-[var(--primary)] bg-[var(--secondary)] px-2.5 py-1 border-2 border-[var(--black)] rounded shadow-[var(--neo-shadow-hover)]">
                  {fmt(principal)}
                </span>
              </div>
              <input
              ...
              <div className="flex justify-between mt-1">
                <span className="text-[9px] font-bold text-[var(--black-muted)] uppercase">
                  R$500
                </span>
                <span className="text-[9px] font-bold text-[var(--black-muted)] uppercase">
                  R$500K
            ...
            <div className="mt-auto flex flex-col gap-3">
              <div className="p-4 bg-[var(--primary)] border-2 border-[var(--black)] rounded-[var(--radius-main)] shadow-[var(--neo-shadow-hover)]">
                <p className="text-[9px] font-black text-[var(--secondary)] uppercase tracking-widest mb-3">
                  Resumo
                </p>
                {[
                  ...
                  {
                    label: "Total aportado (10a)",
                    val: fmtK(principal + monthly * 120),
                  },
                ].map(({ label, val }) => (
                  <div
                    key={label}
                  ...
                  >
                    <span className="text-[10px] font-bold text-[var(--main-bg)] opacity-60 uppercase tracking-wider">
                      {label}:
                    </span>
              ...
              {!isGuest && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 bg-[var(--secondary)] text-[var(--primary)] border-2 border-[var(--black)] rounded-[var(--radius-main)] font-black text-xs uppercase tracking-wider shadow-[var(--neo-shadow-hover)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all cursor-pointer"
                >
          ...
          <div className="lg:col-span-6 flex flex-col">
            <div className="grid grid-cols-3 border-b-2 border-[var(--black)]">
              {calcResults.map(
                (
                  ...
                  <div
                    key={key}
                    className={`${bg} flex flex-col justify-between p-5 ${i < 2 ? "border-r-2 border-[var(--black)]" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest opacity-70 ${textCls}`}
                      >
                        {label}
                      </span>
                      <span
                        className={`text-[14px] font-black px-2 py-0.5 border-2 border-[var(--black)] rounded uppercase tracking-wider shadow-[var(--neo-shadow-hover)] ${badge}`}
                      >
                        +{pct.toFixed(1)}%
                      </span>
                    </div>
                    <div>
                      <p
                        className={`text-[12px] font-black uppercase tracking-wider opacity-60 mb-0.5 ${textCls}`}
                      >
                        Total Final
                      </p>
                      <span
                        className={`text-3xl font-black tracking-tighter leading-none ${textCls}`}
                      >
                        {fmtK(total)}
                      </span>
                    </div>
                    <div
                      className={`mt-4 pt-3 border-t-2 ${divider} border-dashed`}
                    >
                      <div className="flex justify-between items-center">
                        <span
                          className={`text-[9px] font-bold uppercase tracking-wider opacity-60 ${textCls}`}
                        >
                          Juros:
                        </span>
                        <span className={`text-xs font-black ${textCls}`}>
                          {fmtK(juros)}
                        </span>
                      </div>
                    </div>
            ...
            <div className="flex-1 p-4 flex flex-col">
              <p className="text-[10px] font-black text-[var(--black-muted)] uppercase tracking-widest mb-3">
                Curva de Crescimento — Aportado vs Total
              </p>
          ...
          <div className="flex items-center gap-3">
            {investments.length > 0 && (
              <div className="flex items-center gap-3 border-r-2 border-white/20 pr-4">
                ...
                <div>
                  <p className="text-[9px] font-black text-white/60 uppercase tracking-wider">
                    Total
                  </p>
          ...
          {investments.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <p className="text-xs font-bold text-[var(--black-muted)] text-center max-w-xs">
                Adicione seus investimentos para acompanhar o patrimônio e as
              ...
              {!isGuest && (
                ...
                >
                  <Plus size={14} strokeWidth={3} /> Adicionar primeiro
                  investimento
                </button>
              )}
            </div>
