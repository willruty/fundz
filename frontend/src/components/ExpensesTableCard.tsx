        ...
        <div className="p-5 overflow-y-auto flex flex-col gap-5">
          ...
          <div className="bg-[var(--secondary)]/20 border-2 border-[var(--black)] rounded-xl p-4">
            ...
            <p className="text-[11px] text-[var(--black-muted)] leading-relaxed">
              ...
              , <strong>amount/valor</strong>
              {allowTypePerRow && (
                <>
                  , <strong>type/tipo</strong> (income/expense)
                </>
              )}
              <br />
              Ex:{" "}
...
export function TransactionTableCard({
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterDateFrom)
      result = result.filter((t) => t.isoDate >= filterDateFrom);
    if (filterDateTo) result = result.filter((t) => t.isoDate <= filterDateTo);
    if (filterCategory)
      result = result.filter((t) => t.category === filterCategory);
    if (isAllMode && filterType)
      result = result.filter((t) => (t.type ?? "expense") === filterType);
    ...
    <div className="flex flex-col bg-white border-2 border-[var(--black)] rounded-[var(--radius-card)] shadow-[var(--neo-shadow)] overflow-hidden hover:shadow-[var(--neo-shadow-hover)] hover:translate-y-[2px] hover:translate-x-[2px] transition-all duration-200 relative">
      ...
      <div className="bg-[var(--primary)] border-b-2 border-[var(--black)] px-6 py-4 flex flex-col gap-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h3
              className={`text-[10px] font-extrabold tracking-widest ${subtitleColor} uppercase mb-1`}
            >
              {headerSubtitle}
            </h3>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex flex-col items-end mr-2">
              ...
              <span
                className={`text-lg font-black ${totalColor} tracking-tight`}
              >
                {isAllMode && totalAmount >= 0 ? "+" : ""}
                {fmt(totalAmount)}
              </span>
            </div>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            {[
                              ...
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  setFilterType(opt.val as TypeFilter);
                                  setCurrentPage(1);
                                }}
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b-2 border-[var(--black)] bg-white">
              ...
              <th className="p-4 text-[10px] font-black uppercase text-[var(--black-muted)] tracking-widest">
                Data
              </th>
              <th className="p-4 text-[10px] font-black uppercase text-[var(--black-muted)] tracking-widest">
                Descrição
              ...
              <th className="p-4 text-[10px] font-black uppercase text-[var(--black-muted)] tracking-widest text-right">
                Valor
              </th>
              <th className="p-4 text-[10px] font-black uppercase text-[var(--black-muted)] tracking-widest text-center">
                Ações
              </th>
            </tr>
          </thead>
              ...
              <tr className="bg-emerald-50 border-b-2 border-[var(--black)]">
                ...
                <td className="p-4">
                  <CategoryDropdown
                    ...
                    onChange={(catId) =>
                      setEditForm({ ...editForm, category_id: catId ?? "" })
                    }
                  ...
                  >
                    ...
                    <td className="p-4">
                      {isEditingThis ? (
                        ...
                        <CategoryDropdown
                          ...
                          onChange={(catId) =>
                            handleInlineCategoryChange(t, catId)
                          }
                          onCategoriesChange={onMutate}
                    ...
                    <td className="p-4 text-right">
                      {isEditingThis ? (
                        ...
                        <span
                          className={`text-sm font-black px-2 py-1 rounded border-2 border-[var(--black)] ${amountTextClass}`}
                        >
                          {isAllMode && amountSign}
                          {fmt(t.amount)}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex justify-center gap-2">
                        {isEditingThis ? (
                          ...
                          <>
                            <button
                              onClick={() => !isGuest && startEdit(t)}
                              disabled={isGuest}
                              className="p-1.5 rounded border-2 border-transparent hover:border-[var(--black)] hover:bg-white transition-all hover:shadow-[var(--neo-shadow-hover)] text-[var(--black-light)] hover:text-[var(--primary)] cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:border-transparent disabled:hover:bg-transparent disabled:hover:shadow-none"
                            >
                              <Pencil size={15} strokeWidth={2.5} />
                            </button>
                            <button
                              onClick={() => !isGuest && setDeleteTarget(t)}
                              disabled={isGuest}
                              className="p-1.5 rounded border-2 border-transparent hover:border-[var(--black)] hover:bg-white transition-all hover:shadow-[var(--neo-shadow-hover)] text-[var(--black-light)] hover:text-red-500 cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:border-transparent disabled:hover:bg-transparent disabled:hover:shadow-none"
                            >
