        ...
        <div className="flex bg-gray-100/50 dark:bg-gray-800/50 p-1.5 m-6 rounded-button relative">
          {(["login", "register"] as AuthMode[]).map((tab) => (
            <button
              key={tab}
              onClick={() => setMode(tab)}
              className={`flex-1 py-2 text-xs font-extrabold tracking-widest relative isolate  hover:cursor-pointer transition-colors duration-500 ${
                mode === tab
                  ? "text-primary"
                  : "text-primary/40 dark:text-text-dark/40"
              }`}
            >
              {tab.toUpperCase()}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <AnimatePresence initial={false}>
              {mode === "register" && (
                ...
                >
                  <label className="text-[10px] font-extrabold text-primary dark:text-secondary uppercase tracking-widest mb-1.5 ml-1">
                    Nome Completo
                  </label>
                  <input
                    type="text"
            ...
            {mode === "login" && (
              <div className="flex justify-end">
                <a
                  href="#"
                  className="text-xs font-bold text-primary dark:text-secondary hover:underline underline-offset-4"
                >
                  Esqueceu a senha?
                </a>
              </div>
